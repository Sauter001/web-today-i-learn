...
2. attendance 테이블에서 중복을 제거하기 위해 crew 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
   crew_id (primary key)와 nickname 컬럼을 가진 별도의 테이블을 만든다
...
```sql
ALTER TABLE attendance
ADD CONSTRAINT fk_crew FOREIGN KEY (crew_id) REFERENCES crew(crew_id);
```
...
```sql
ALTER TABLE crew
ADD CONSTRAINT uc_email UNIQUE(nickname)
```
...
FROM attendance
WHERE
    crew_id = (SELECT crew_id FROM crew WHERE nickname = '어셔')
...
VALUES ((SELECT crew_id FROM crew WHERE nickname = '어셔'), '2025-03-06', '09:31', '18:01');
```
...
WHERE
    crew_id = (SELECT crew_id FROM crew WHERE nickname = '주니')
...
검프는 SQL이 익숙치 않아 crew 테이블에서 먼저 닉네임을 검색하고 해당 아이디 값을 찾아 직접 WHERE문에서 crew_id 항목의 값을 수동으로 입력해서 출석 기록을 조회했다. 그런데 crew 테이블에서 crew_id를 기준으로 nickname 필드 값을 가져와서 함께 조회할 수도 있지 않을까?
...
FROM crew c JOIN attendance a ON c.crew_id = a.crew_id
...
```sql
SELECT crew_id, count(attendance_date) recorded_days
FROM attendance
GROUP BY crew_id;
```
...
'크루별로'라는 단어를 고려해 등교 기록이 없는 크루는 0으로 카운트되도록 함.

```sql
SELECT crew_id, count(start_time) recorded_days
FROM attendance
GROUP BY crew_id
```
...
```sql
SELECT attendance_date, COUNT(crew_id) crew_count
FROM attendance
